...
def __find_fill_datadate(driver: WebDriver, data_fill: data_fill_jira):
    while True:
        ...
        month_mon = int(element_mon.find_element(
            # type: ignore
        ...
        day_sun = element_sun.find_element( 
            By.CLASS_NAME, value="day-Num").get_attribute("textContent")
        month_sun = int(element_sun.find_element(
            # type: ignore
            By.CLASS_NAME, value="month-Num").get_attribute("textContent"))+1
...
def __fill_task_data(driver: WebDriver, data_fill: data_fill_jira):
    try:
        ...
        raise NoSuchElementException(msg=f"this Datetime {data_fill.filldatetime.strftime(
    ...
    Select(pnladdedit_timelist.find_element(
    ...
    pnladdedit_timelist.find_element(By.ID, value="cphContent_txtInternalDescription").send_keys(data_fill.description_str())
    driver.find_element(
...
def main_filldatatask(driver: WebDriver, data_fill_list: list[data_fill_jira]) -> list[data_fill_jira]:
...
def main_submitatask(driver: WebDriver, data_fill_list: list[data_fill_jira]) -> list[data_fill_jira]:
    for data_fill in data_fill_list:
        try:
            __find_fill_datadate(driver, data_fill)
            __submit_time_sheet(driver)
        except Exception as e:
            data_fill.status_message = f"{data_fill.status_message} | Submit Error : {str(e)}"
    return data_fill_list
